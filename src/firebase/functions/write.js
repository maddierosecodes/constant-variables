...
export const writeUserData = (document, path, uid) => {
  return setDoc(doc(db, path, uid), document).catch((err) =>
...
export const writeListingData = (document, path) => {
  return addDoc(collection(db, path), document).catch((err) =>
    console.log(err)
  );
};
...
export const postAdvert = (isDriver, document = exampleListing) => {
  ...
  return addDoc(collection(db, path), document).catch(console.log);
};
